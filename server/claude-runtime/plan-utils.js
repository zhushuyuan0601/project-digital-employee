...
function normalizeParticipants(participants = [], workflow = []) {
  ...
  for (const agentId of workflow.map((node) => node.assignedAgentId).filter(Boolean)) {
    ...
    byAgent.set(agentId, {
      agentId,
      needed: true,
      reason: `该 Agent 的能力卡匹配本次任务中的 ${agent?.roleName || agentId} 节点。`,
    })
  }
...
function normalizeWorkflowNode(raw, index, knownIds) {
  ...
  const phase = String(raw?.phase || raw?.intent || 'route').trim() || 'route'
  const validAgents = executableAgentIds()
  ...
  const acceptanceCriteria = stringArray(raw?.acceptanceCriteria?.length ? raw.acceptanceCriteria : raw?.successCriteria)

  return {
    id,
    title: String(raw?.title || '').trim(),
    phase,
    intent: String(raw?.intent || phase || 'general').trim() || 'general',
    requiredCapabilities: stringArray(raw?.requiredCapabilities?.length ? raw.requiredCapabilities : raw?.agentCapabilityHints),
    assignedAgentId,
    ...
    inputArtifacts: stringArray(raw?.inputArtifacts),
...
function normalizeLegacyPlan(plan) {
  ...
  const workflow = subtasks.map((subtask, index) => ({
    ...
    intent: subtask.intent || subtask.phase || 'review',
    requiredCapabilities: stringArray(subtask.requiredCapabilities?.length ? subtask.requiredCapabilities : subtask.agentCapabilityHints),
    ...
    inputArtifacts: stringArray(subtask.inputArtifacts),
    expectedOutputArtifacts: stringArray(subtask.expectedOutputArtifacts),
