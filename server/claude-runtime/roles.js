...
- 没有合适 Agent 时，在 planningNotes 中说明能力缺口，并选择最接近的通用/既有 Agent。
- participants 只列出本次任务实际需要的 Agent，并写明路由原因。
- workflow 是 1-N 个能力节点 + DAG。每个节点尽量写清 intent、requiredCapabilities、assignedAgentId、routingReason、dependsOn、inputArtifacts、expectedOutputArtifacts、executionMode、successCriteria、acceptanceCriteria。
...
- 每个节点的 expectedOutputArtifacts 和 acceptanceCriteria 应能支撑下游验收。
